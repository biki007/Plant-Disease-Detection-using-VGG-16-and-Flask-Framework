...
def predict():
    if request.method == 'POST':
        ...
        sym=dict2.get(b)
        prev=dict3.get(b)
        cursor.execute("INSERT INTO result1 (prediction) VALUES (%s)", (b,))
        conn.commit()
        cursor.close()
        conn.close()
...
def details():
    ...
    data = []  # Initialize an empty list outside the loop
    for row in rowset:
        ele = []
        count = 1
    ...
    print(data)  # Print outside the loop
    data = pd.DataFrame(data, columns=['Disease', 'Count'])  # Convert to DataFrame outside the loop
    
    colors = {'Pepper_bell_Bacterial_spot': 'red', 'Pepperbell_healthy': 'yellow', 'Potato_Early_blight': 'blue', 'Potato_Late_blight': 'green', 'Potato__healthy': 'magenta', 'Tomato_Bacterial_spot': 'cyan', 'Tomato_Early_blight': 'orange', 'Tomato_Late_blight': 'purple', 'Tomato_Leaf_Mold': 'olive', 'Tomato_Septoria_leaf_spot': 'brown', 'Tomato_Two_spotted_spider_mite': 'coral', 'Tomato_Target_Spot': 'khaki', 'Tomato_Yellow_Leaf_Curl_Virus': 'seagreen', 'Tomato_mosaic_virus': 'aqua', 'Tomato_healthy': 'silver'}
    labels = list(colors.keys())
    handles = [plt.Rectangle((0, 0), 1, 1, color=colors[label]) for label in labels]
    c = data['Disease'].apply(lambda x: colors[x])
    plt.legend(handles, labels, loc='upper right')
    plt.bar(data['Disease'], data['Count'], color=c)
    plt.xlabel('Labels')
    plt.ylabel('Counts')
    plt.xticks([])
